...
namespace Iris.Web.HttpCompress
{
    ...
    /// <remarks>
    ///     <p>
    ///         This HttpModule uses classes that inherit from <see cref="CompressingFilter" />.
    ...
    ///         class that inherits from HttpFilter to do the work.
    ///     </p>
    ///     <p>
    ...
    public sealed class HttpModule : IHttpModule
    {
        ...
        {
            if (app == null || app.Response == null || string.IsNullOrWhiteSpace(app.Response.ContentType))
                return false;
        ...
        private static float GetQuality(string acceptEncodingValue)
        {
            ...
            if (qParam >= 0)
            {
                ...
                {
                    val =
                        float.Parse(acceptEncodingValue.Substring(qParam + 2, acceptEncodingValue.Length - (qParam + 2)));
                }
