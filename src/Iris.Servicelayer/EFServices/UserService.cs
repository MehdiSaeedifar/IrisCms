...
namespace Iris.Servicelayer.EFServices
{
    public class UserService : IUserService
    {
        ...
        public int Count
        {
            get { return _users.Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromHours(1)).Count(); }
        }
        ...
        public EditedUserStatus UpdateProfile(EditProfileModel userModel)
        {
            ...
            {
                User selectedUser = _users.Include(user => user.UserMetaData)
                    .FirstOrDefault(user => user.UserName.Equals(userModel.UserName));
        ...
        public int GetLastMonthActiveUsersCount()
        {
            ...
            return _users.Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromHours(1)).Count(u => u.LastActivity >= firstDay);
        }
